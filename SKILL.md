...
```json
{
  ...
  "client_id": 1,
  "readonly": false,
  "comment": "IB Gateway IP 和 Port"
}
```

- `host`: IB Gateway/TWS 的 IP（用遠端時填內網 IP）
- `port`: API Port（IB Gateway 預設 4001，TWS 預設 7497）
- `client_id`: 固定 client ID（避免重複連線）
- `readonly`: `true` = 唯讀，`false` = 可下單

### 2. IB Gateway 設定

1. 開啟 IB Gateway → Settings → API
2. 勾選 **Enable ActiveX and Socket Clients**
3. 設定 **Socket Port**（預設 4001）
4. 確認 **Paper Trading** 或 **Live** 帳戶有 API 權限
...
將此資料複製到 OpenClaw 的 skills 目錄：

```bash
cp -r openclaw-wheel ~/.openclaw/skills/
```
...
A: 
1. 檢查 IB Gateway API 設定是否有勾選 Enable API
2. 確認市場是否開盤（盤後訂單會排隊）
3. 檢查 Paper Trading 帳戶是否開通 API 權限
